let owmapi = (function (owmapi, $) {
    ...
    owmapi.one_call_weather = (cur_weather_dom, week_weather_dom) => {
        let weather
        send(
            ...
            dom.empty().append(
                $("<div>").addClass("row").append(
                    ...
                    $("<div>").addClass("col-md-12").append(
                        $("<h5>").addClass("align-font-txt-center").html("CURRENT")
                    ),
        ...
        if (week_weather_dom !== undefined) {
            ...
            let row_dom = $("<div>").addClass("row");
            for (let i = 1; i < 7; i++) {
                // IMPORTANT!! .align-font-txt-center는 사용자 정의 스타일
                row_dom.append(
                    $("<div>").addClass("col-md-2").append(
                        $("<div>").addClass("col-md-12").append(
                            $("<h5>").addClass("align-font-txt-center").html((i === 0)? "TODAY" : get_day_name_from_unix_timestamp(weather.daily[i].dt))
                        ),
                        $("<div>").addClass("col-md-12").addClass("text-center").append(
